...
4. xAI structured extraction decides whether supported ingredients and actionable steps exist. Social sources collect original captions, independently selected video/audio streams, and up to 24 timestamped frames before normalization. Audio is transcribed when captions are unavailable. Public post images and page-embedded video are additional fallbacks. Translation and recipe output use the saved recipe/chef language. An optional native YouTube reader supplies original speech and chronological video observations when server-side media access fails. Exact-source web research can fill remaining gaps; unsupported recipes are not saved. A second evidence-checking pass reconciles ingredient quantities, action order and safety steps. See [recipe import diagnostics and deployment](RECIPE_IMPORTS.md).
...
### Cookbook deletion and Chinese captions — September 21

Recipe details now offer a trash button and a native confirmation alert. The authenticated deletion endpoint atomically cancels the associated import and replaces the cookbook document with a small deletion marker, removing version 1. The marker prevents stale/offline writes from restoring a deleted recipe; listeners clear it from local recipes and pending edits. Cooking attempts retain their recipe snapshots and cover images. Re-importing the same URL allocates a new attempt and may replace the marker. Ship the Firestore rule and backend endpoint before the new iOS build.

Read-only inspection of import `9a6271869d16ebd67ee3d8a7a1b0a3d4` for [the braised-beef video](https://www.youtube.com/watch?v=d31CCyGSGZA) found `scope: food`, `status: skipped`, and `hasTranscript: false`. Its extraction reason correctly identified ingredients without a cooking sequence; retrieval reported no caption tracks. Unlike the older September 20 attempt discussed above, classification and extraction ran. The worker's yt-dlp call returned metadata successfully, so the exception-only HTML fallback never ran. The public player page exposes Chinese and English caption tracks.

Social extraction now tries public-page captions when yt-dlp returns missing or unreadable captions, before attempting audio. Both retrieval paths share caption ranking: prefer original-language/untranslated evidence and try distinct tracks before alternate encodings. Manual and automatic tracks no longer overwrite each other. [yt-dlp's upstream report](https://github.com/yt-dlp/yt-dlp/issues/13831) also documents problems with auto-translated tracks. No browser cookies, new transcript provider, or authentication bypass was added.

Imports now retain the source metadata, last incomplete extraction stage, partial ingredients, original transcript, English caption or translation, and sampled frame timestamps for the import detail screen. When written evidence is insufficient, videos up to 20 minutes are sampled evenly at up to 24 frames even when captions exist, so on-screen quantities and cooking actions reach the same extraction request as the transcript.

JSON3 caption cues are converted from milliseconds to explicit seconds before reaching the model. Captions get a separate input block so long written pages cannot truncate them. The extraction prompt explicitly translates Chinese and other source languages into English; schema descriptions reinforce complete steps, pressure-cooking details, timer units, and video cue units. Prompt version: `companion-6`.

Live verification recovered the exact video's `zh-CN` captions. Three extraction calls used one reserved diagnostic import attempt; the first exposed millisecond timestamps mistakenly copied as seconds, which led to deterministic cue normalization. Final run `cda3ea76-628a-4e98-9406-8ad03d8ebba0` returned 15 ingredients and six English steps, with in-range video timestamps and separate 1,500-second and 180-second pressure-cooking timers. The output retains pressure buildup, pressure release, and the cooking sequence. Evidence/output: `/private/tmp/oui-beef-social.json`, `/private/tmp/oui-beef-english.json`, `/private/tmp/oui-beef-recheck-final.log`. This was local extraction using the live provider, not a Cloud Run acceptance test; source access can still vary by server/IP. No user cookbook or import records were rewritten, and no deployment or TestFlight upload was performed.

Verification: 29 backend tests passed (three unrelated emulator-only tests skipped); all 16 companion tests passed with isolated Firestore/Storage emulators, including owner isolation, rejection of stale saves to deleted recipes, and server replacement on re-import. All 30 Swift core tests passed. The original Swift cache had conflicting `/tmp` and `/private/tmp` paths; a fresh canonical cache resolved the compiler failure.

The iOS simulator deletion journey passed: cancel leaves the recipe intact, confirm removes it, and the existing cooking attempt remains resumable. The first test revealed iOS 26 adapting the confirmation dialog to a popover without a visible Cancel action; an explicit native alert resolved that. Result bundle: `/private/tmp/oui-delete-ui-final.xcresult`.

History reset now gives a newly created import an epoch-based attempt value rather than reusing attempt 1. This avoids Cloud Tasks deduplication and stale worker matches after an import record is removed; retrying an existing job still increments its prior attempt. The backend regression test exercises both cases.

### Deployment and requested history reset — September 21

After user authorization, the tested source snapshot was built by Cloud Build `ce7ea03f-6aea-44df-82ff-cc6e685495ce` and deployed as `oui-chef-voice-history-c1cbdf2fd553`, serving 100% of the existing service traffic. Image digest: `sha256:d8ac8488ed4f7279bec34a249f2715fc5b8e6fe32147678019d3d5a8c89c7969`. Firestore rules deployed successfully. Environment/secrets, runtime identity, CPU/memory, concurrency and timeout were preserved. Health returned 200; unauthenticated import/deletion returned 401, and unsigned workers returned 403. The existing iPhone installation uses this service; no additional app build is required.

The account owning the reported braised-beef import was verified through Firebase Auth. Its two failed import records and five AI run records were deleted with update-time preconditions. It had no stored question conversations or question counter. Saved recipes, cooks, preferences, photos, other accounts and the shared import allowance were preserved. Deployment and reset receipts are in `/private/tmp/oui-deployment-check.json` and `/private/tmp/oui-history-reset-receipt.json`. No new import or paid AI request was made during deployment/reset verification.
...
Committed source `60a58f6` was built by Cloud Build `0362378c-11b5-4d14-b553-e3f4809c3fed` and deployed as `oui-chef-voice-layered-60a58f6`, serving 100% of traffic. Image digest: `sha256:242649df4b84254c84b6c983a86da48598a15bf5dda9fa46c899b841664d2ed4`. The existing environment, runtime identity, 900-second timeout, concurrency, secrets, account data, and quotas were preserved. Health returned 200; unauthenticated imports returned 401 and unsigned worker calls returned 403. No paid AI call was made for deployment validation.
