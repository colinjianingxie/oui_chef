...
| `recipeSets/{setID}` | Chef reference, name, classifications, publication status, and free/subscription price metadata. |
| `recipes/{recipeID}` | Lightweight card metadata, chef/set references, classification IDs, published version, and whether an owner-only draft exists. |
...
| `ingredientCategories/{categoryID}` | Hierarchical ingredient categories, separate from recipe classifications. |
| `classifications/{classificationID}` | Reusable cuisine, dish, meal, technique, skill, occasion, and collection classifications. Both recipes and sets reference these IDs. |
...
Future paid sets use recurring subscriptions, as previously selected. Price metadata initially supports USD amounts in integer cents and monthly/yearly intervals. StoreKit product configuration, purchase verification, localized prices, renewals, and refunds must be connected before sales are enabled. Paid graph reads fail closed without an entitlement.
...
```sh
swift test
firebase emulators:start --only firestore,auth --project demo-ouichef
# In another terminal:
...
Search uses bounded name/title/alias/category prefixes and individual words, not fuzzy full-text ranking. Recipe and ingredient results use cursors (12 cards or six ingredient choices per page). Ingredient searches reset the category filter; category selection resets search. Taxonomy documents are small and loaded together; introduce taxonomy pagination if those lists become large. StoreKit checkout, chef authoring screens and image uploads remain separate future work.
