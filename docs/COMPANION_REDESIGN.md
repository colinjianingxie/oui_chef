...
Retrieval is deterministic before recipe AI: source host selects public HTML extraction or yt-dlp's platform extractor. For social sources, retrieve metadata and timestamped captions first; if captions are unavailable, attempt bounded media download, audio extraction/transcription, and key frames before parsing. Reuse this evidence rather than asking the model whether to run the downloader again. The existing limits remain 20 minutes and 40 MB for downloadable media. Login-only, unavailable, or unsupported media can fall back to public page evidence or pasted recipe text; successful extraction from every platform link is not guaranteed. Tool reference: [yt-dlp options](https://github.com/yt-dlp/yt-dlp#usage-and-options).
