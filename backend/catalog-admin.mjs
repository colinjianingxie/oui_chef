...
import { catalogProject, validID, draftRevision, cardFor, canonicalTags, canonicalAccess, publishRecipe } from './catalog.mjs';
...
function checkSet(set) {
  if (!validID(set.id) || !validID(set.chefID) || typeof set.title !== 'string' || !['draft', 'published'].includes(set.status)) throw new Error('Invalid recipe set');
  set.access = canonicalAccess(set.access);
  set.tags = canonicalTags(set.tags);
  delete set.price; delete set.classificationIDs;
}
if (command === 'seed') {
  ...
  for (const food of library.foods) {
    ...
    await createOnce(`ingredients/${food.id}`, { ...food, categoryAncestors: ancestors });
  }
  for (const recipe of recipes) {
    const classes = canonicalTags(recipe.tags);
    if (!validID(recipe.id) || !seed.recipeSets.some(s => s.id === recipe.recipeSetID && s.chefID === recipe.chefID)) throw new Error('Invalid recipe references');
  ...
  recipe.tags = canonicalTags(draft.tags ?? recipe?.tags);
  if (!validID(recipe?.id) || !validID(recipe.chefID) || !validID(recipe.recipeSetID)) throw new Error('Invalid import references');
  ...
  await db.runTransaction(async tx => {
    ...
    else tx.create(ref, cardFor(recipe, recipe.tags, 'draft', true));
...
} else throw new Error('Supported commands: seed, draft, publish');
